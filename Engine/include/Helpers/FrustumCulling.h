...
public:
    ...
    Mesh::AABBssbo transformAABB(const Mesh::AABBssbo& localAABB,
                                 const glm::mat4& modelMatrix) const {
        auto worldCenter = glm::vec3(modelMatrix * localAABB.center);
    ...
    bool isAABBInFrustum(const Mesh::AABBssbo& worldAABB) const {
        for (const auto& plane : planes) {
            glm::vec3 normal(plane);
            glm::vec3 absNormal = glm::abs(normal);
            float projectedRadius = dot(glm::vec3(worldAABB.extents), absNormal);
            float distance = dot(normal, glm::vec3(worldAABB.center)) + plane.w;
