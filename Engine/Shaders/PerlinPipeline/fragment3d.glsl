...
const float frequenceMul[6u] = float[](2.0, 8.0, 14.0, 20.0, 26.0, 32.0);
...
{
	vec3 coord = vec3(float(pixel.x) / WIDTH, float(pixel.y) / WIDTH, float(pixel.z) / WIDTH);
...
void main() {
    // Normalize FragPos to map it properly on the unit cube
    vec3 direction = normalize(FragPos);

    // Output grayscale
    FragColor = stackable3DNoise(direction);
}