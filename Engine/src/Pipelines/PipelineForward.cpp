...
bool create = false;
MultiDrawIndexedAttribs commands;
std::vector<MultiDrawIndexedItem> items;
    ...
    if (!meshes.empty())
    {
	    if (!create)
	    {
            ...
            for (auto& mesh : meshes)
            {
                ...
                item.BaseVertex = currentVertex;
                item.FirstIndexLocation = currentIndex;
                item.NumIndices = mesh->verticesData().indices.size();
            ...
            commands.FirstInstanceLocation = 0;
            commands.Flags = DRAW_FLAGS::DRAW_FLAG_VERIFY_STATES;
            commands.IndexType = VT_UINT32;
            commands.NumInstances = 1;
            commands.pDrawItems = items.data();
            ...
            IndirectBufferDesc.Size = sizeof(commands);

            BufferData InitData;
            InitData.pData = &commands;
            InitData.DataSize = sizeof(commands);
        ...
        contextData.m_pImmediateContext->MultiDrawIndexed(commands);
