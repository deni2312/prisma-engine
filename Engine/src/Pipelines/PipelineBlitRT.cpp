...
{
    auto& contextData = Prisma::PrismaFunc::getInstance().contextData();
    auto pRTV = Prisma::PipelineHandler::getInstance().textureData().pColorRTV->GetDefaultView(Diligent::TEXTURE_VIEW_RENDER_TARGET);
    auto pDSV = Prisma::PipelineHandler::getInstance().textureData().pDepthDSV->GetDefaultView(Diligent::TEXTURE_VIEW_DEPTH_STENCIL);
