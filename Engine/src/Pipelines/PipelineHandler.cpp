...
{
	if (Prisma::GlobalData::getInstance().currentGlobalScene() && Prisma::GlobalData::getInstance().currentGlobalScene()
		->camera)
	{
		...
		m_deferredForwardPipeline = std::make_shared<PipelineDeferredForward>(
			settings.width, settings.height, sceneParameters.srgb);
		return true;
	}
	return false;
}
