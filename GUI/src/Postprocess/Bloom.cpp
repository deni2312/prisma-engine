...
void Prisma::GUI::Bloom::createShaderPingPong() {
    ...
    PSOCreateInfo.PSODesc.ResourceLayout.ImmutableSamplers = ImtblSamplers;
    PSOCreateInfo.PSODesc.ResourceLayout.NumImmutableSamplers = _countof(ImtblSamplers);
    contextData.device->CreateGraphicsPipelineState(PSOCreateInfo, &m_psoPingPong);

    m_psoPingPong->GetStaticVariableByName(Diligent::SHADER_TYPE_PIXEL, "Constants")->Set(m_pingPong);
    Diligent::TextureDesc RTColorDesc;
    RTColorDesc.Name = "Offscreen render target";
    RTColorDesc.Type = Diligent::RESOURCE_DIM_TEX_2D;
    RTColorDesc.Width = contextData.swapChain->GetDesc().Width;
    ...
    m_psoPingPong->CreateShaderResourceBinding(&m_srbPingPong, true);
    m_srbPingPong->GetVariableByName(Diligent::SHADER_TYPE_PIXEL, "screenTexture")->Set(PipelineHandler::getInstance().textureData().pColorRTV->GetDefaultView(Diligent::TEXTURE_VIEW_SHADER_RESOURCE));
...
void Prisma::GUI::Bloom::createShaderRender() {
    ...
    m_psoRender->GetStaticVariableByName(Diligent::SHADER_TYPE_PIXEL, "hdrTexture")->Set(m_texturePingPong[0]->GetDefaultView(Diligent::TEXTURE_VIEW_SHADER_RESOURCE));
...
void Prisma::GUI::Bloom::renderPingPong() {
    ...
    for (unsigned int i = 0; i < amount; i++) {
        ...
        contextData.immediateContext->SetPipelineState(m_psoPingPong);
        ...
        contextData.immediateContext->SetIndexBuffer(quadBuffer.iBuffer, 0, Diligent::RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
        {
            ...
            Diligent::MapHelper<glm::ivec4> constants(contextData.immediateContext, m_pingPong, Diligent::MAP_WRITE, Diligent::MAP_FLAG_DISCARD);
            *constants = glm::ivec4(horizontal, 0, 0, 0);
        }
        m_srbPingPong->GetVariableByName(Diligent::SHADER_TYPE_PIXEL, "screenTexture")
                     ->Set(first_iteration ? m_textureBrightness->GetDefaultView(Diligent::TEXTURE_VIEW_SHADER_RESOURCE) : m_texturePingPong[!horizontal]->GetDefaultView(Diligent::TEXTURE_VIEW_SHADER_RESOURCE));

        // Set texture SRV in the SRB
        contextData.immediateContext->CommitShaderResources(m_srbPingPong, Diligent::RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
