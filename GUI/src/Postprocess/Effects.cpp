...
Prisma::Effects::Effects() {
	...
	m_shaderBrightness = std::make_shared<Shader>("../../../GUI/Shaders/Brightness/vertex.glsl", "../../../GUI/Shaders/Brightness/fragment.glsl");
	...
	for (unsigned int i = 0; i < 2; i++)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, settings.width, settings.height, 0, GL_RGBA, GL_FLOAT, NULL);
	...
	m_brightnessFbo = std::make_shared<Prisma::FBO>(fboData);
	Prisma::TextureInfo::getInstance().add({ pingpongColorbuffers[0],"Bloom1"});
	Prisma::TextureInfo::getInstance().add({ pingpongColorbuffers[1],"Bloom2" });
}
...
void Prisma::Effects::render(std::shared_ptr<Prisma::FBO> texture, std::shared_ptr<Prisma::FBO> raw) {
	switch (m_effects) {
	case EFFECTS::NORMAL:
		break;
	case EFFECTS::SEPPIA:
